...
The NoMa buildings OBJ was exported from Blender with **FCC HQ (45 L St NE) at
local origin (0,0)**, but the voxelized `manifest_3d.json` was missing
`epsg`/`merc_anchor`, so `voxelize_city.lonlat_to_vox` failed. This computes
`merc_anchor = EPSG:3857(FCC HQ)` and injects it, which makes lat/lon ↔ voxel
exact.

Verified: the 5 live base stations from base_stations.csv reproduce the catalog
`distance_m` from FCC to **< 1 m** (see `verify()`), confirming the anchor + the
manifest's `merc_scale`/`origin_units` are self-consistent.

  python city_georef.py --patch     # write epsg+merc_anchor into the city manifest (keeps .bak)
...
def _live_sites():
    """Unique live donor sites from the catalog: name → (lon, lat, distance_m)."""
    sites = {}
...
def verify():
    ...
    print(f"{'site':16s} {'vox(x,z)':>14s}  in  {'grid_d':>7s} {'catalog':>7s} {'err':>6s}  near-bldg")
    ok = True
    for sid, (lon, lat, dcat, name) in _live_sites().items():
        ...
        # nearest building (class 3) within a small window at street level
        near = False
        if inb:
            x0, z0 = int(np.clip(vx - 6, 0, NX)), int(np.clip(vz - 6, 0, NZ))
            near = bool((grid[x0:x0 + 12, 2, z0:z0 + 12] == 3).any())
        err = d - dcat
        ok &= inb and abs(err) < 5.0
        print(f"{sid:16s} ({vx:6.0f},{vz:6.0f}) {str(inb)[0]}  {d:7.1f} {dcat:7.1f} {err:+6.1f}  {near}")
