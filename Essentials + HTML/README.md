...
| **`Frontend_Data_Display.html`** | The dashboard entry point — a ~390-line shell (stays at repo root). Serve with `python3 -m http.server 8432` and open in a browser. Loads its UI from `Frontend/` and its data from `Data/`; the Simulator tab needs `SIM/web/`. |
| **`Frontend/`** | The dashboard UI, organized by feature: `2d-3d/` (the Map-Coverage viewer — `dashboard.js`, `viewer3d.js`, `spectrum.js` + css), `simulator/` (sim UI), and `landing/` + `indoor-outdoor/` (placeholders for the eventual chooser / mode split). See [Frontend/2d-3d/README.md](Frontend/2d-3d/README.md) for the classic-vs-module `window.*` contract to preserve when editing. |
| **`Data/`** | Frontend data + models: `records_data.js`, `floorplan_image.js`, `timeseries_data.js` (extracted verbatim from the old inline HTML), and `models/` (the 3D `.obj` folders the viewer loads). See [Data/README.md](Data/README.md). |
