...
def _selftest() -> int:
    ...
    manifest = json.loads((HERE.parent / "manifest_3d.json").read_text())
    M = np.load(HERE.parent / "material_grid.npy")
    inside = np.load(HERE.parent / "inside_mask.npy")
    norm = load_norm(manifest)
    ...
    valid = np.load(HERE.parent / "valid_tx_mask.npy")
...
if __name__ == "__main__":
    ...
    if a.budget:
        ...
        for name in ("inside_mask", "valid_tx_mask"):
            m = np.load(HERE.parent / f"{name}.npy")
